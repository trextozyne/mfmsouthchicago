    ...
    success: function (data) {
        ...
        data.forEach((data, k) => {
            ...
            html+='<tr>\n' +
                ...
                '<img width="6%" class="thumbnail" src="assets/images/pdf-thumbnail.jpg" title="pdf-mfm-thumbnail"/>' + data.filename +'</td>\n' +
                '                <td class="body-item mbr-fonts-style display-7">' +new Date(data.uploadDate).toLocaleDateString('en-US')  +'</td>\n' +
                '                <td class="body-item mbr-fonts-style display-7">' +
                '<a target="_blank"  href="/files/download/'+ data._id +'">Download</a></td>' +
                '</tr>'
        });
        html += '</tbody>\n' +
            '            </table>\n' +
        ...
        let referenceNode = document.getElementById("header1-1z");
        let htmlObject = document.createElement('section');
        htmlObject.setAttribute("id", "table1-36");
        htmlObject.classList.add("section-table");
        htmlObject.classList.add("cid-rcVfb1SSLq");
